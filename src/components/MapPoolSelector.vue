<template>
    <div class="map-pool-select">
        <div v-for="(map, i) in selected" :key="i" :class="map.selected ? `selected` : ``" @click="selected[i].selected = !selected[i].selected">{{ map.name }}</div>
    </div>
</template>
...
<script setup lang="ts">
    ...
    const selected = ref(maps.map((a) => { return {
        name: a.name,

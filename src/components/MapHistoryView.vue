    ...
    <div class="container-log">
        <div v-for="(action, i) in model" :key="i">
            <div class="entry" v-if="action.type == `ban`">
                <div class="banner" :style="`background-image: url(${maps.find((a) => a.name == action.map)?.image});`">
                    <div class="text">{{ action.map?.toUpperCase() }} selected by <span :class="action.team == 1 ? `atk` : `def`"> TEAM {{ action.team }}</span></div>
                </div>
            ...
            <div class="entry" v-if="action.type == `side`">
                <div class="banner" :style="`background-image: url(${maps.find((a) => a.name == $props.finalMap)?.image});`" v-if="finished && i == (model?.length || 0) - 1">
                    <div class="text">Deciding map is {{ $props.finalMap?.toUpperCase() }}</div>
                </div>
                <div class="info bg-pick">
                    <span>TEAM {{ action.team }} PICKED {{ action.side == sides.DEFENSE ? "DEFENSE" : "ATTACK" }}</span>
                    <b-icon-check-circle></b-icon-check-circle>
                </div>
            </div>
        </div>
